    ...
    if queryCount > 1:
        ...
        ohlc.rename(columns={
            ...
            'vw': 'VWAP'
                    ...
                    inplace=True)
        ohlc = ohlc[[
            'Seconds', 'Open', 'High', 'Low', 'Close', 'Volume', 'VWAP'
        ]]
...
def get_position(trading_client, symbol):
    ...
    if position:
        print(
            f"Open position for {symbol}: {position.qty} shares at {position.avg_entry_price}"
        )
    else:
    ...
    if not open_orders:
        ...
        for order in open_orders:
            ...
            try:
                ...
                canceled_frame.to_csv(canceled_file,
                                      mode="a",
                                      header=not os.path.exists(canceled_file),
                                      index=False)
...
# def submit_trade(trading_client, symbol, shares_per_trade, side, type,
#                  limit_price, submits_file):
...
#         print(
#             f"Submitting a {type} {side} order for {shares_per_trade} shares of {symbol}"
#         )
#         order_params = MarketOrderRequest(symbol=symbol,
...
#         print(
#             f"Submitting a {type} {side} order for {shares_per_trade} shares of {symbol} at {limit_price}"
#         )
#         order_params = LimitOrderRequest(symbol=symbol,
#                                          qty=shares_per_trade,
#                                          side=side,
#                                          type=type,
#                                          time_in_force=TimeInForce.DAY,
...
#         print(
#             f"Order submitted for: Symbol={response.symbol}, Type={response.type}, Side={response.side}, shares_per_trade={response.qty}"
#         )
...
#         order_data.to_csv(submits_file,
#                           mode="a",
#                           header=not os.path.exists(submits_file),
#                           index=False)
...
def submit_trade(
    ...
    submits_file: str = None,
):
    """
    Submit a market/limit order and optionally append the response to a CSV.

    Returns:
        response (alpaca.trading.models.Order) on success, or None on failure.
    """
    tzone = ZoneInfo("America/New_York")
    ...
    if order_type == "market":
        print(f"Submitting a MARKET {side} for {qty} {symbol}")
        ...
        print(f"Submitting a LIMIT {side} for {qty} {symbol} at {limit_price}")
    ...
    try:
        response = trading_client.submit_order(order_params)
        print(f"Order submitted: id={response.id} symbol={response.symbol} "
              f"side={response.side} qty={response.qty} type={response.type}")
        ...
        print(f"Trade order rejected: {e}")
        # log error line if you still want to
        if submits_file:
            pd.DataFrame([{
                "error": str(e),
...
def plot_ema(price_frame):
    ...
    price_frame["upper_band"] = price_frame[
        "ema_price"] + bollinger_width * price_frame["volatility"]
    price_frame["lower_band"] = price_frame[
        "ema_price"] - bollinger_width * price_frame["volatility"]

    # Plot the EMA price and the Bollinger Bands
    plt.figure(figsize=(20, 12))
    plt.plot(price_frame["timestamp"],
             price_frame["price"],
             label="Price",
             color="blue")
    plt.plot(price_frame["timestamp"],
             price_frame["ema_price"],
             label="EMA Price",
             color="red")
    plt.plot(price_frame["timestamp"],
             price_frame["upper_band"],
             label=f"Upper + {bollinger_width} STDV",
             color="orange",
             linestyle='--')
    plt.plot(price_frame["timestamp"],
             price_frame["lower_band"],
             label=f"Lower - {bollinger_width} STDV",
             color="orange",
             linestyle='--')
    plt.title(
        f"{symbol} EMA Prices and Bollinger Bands with α = {alpha_param} / {date_pretty}",
        fontsize=20)
...
class EMACalculator:
    ...
    def __init__(self, alpha_param):
        ...
        self.price_frame = pd.DataFrame(columns=[
            "timestamp", "symbol", "price", "volume", "ema_price", "volatility"
        ])
    ...
    def calc_ema(self, trade_price, trade_size, time_stamp):
        ...
        global price_var
        price_var = alpha_squared * price_var + alpha2 * (trade_price -
                                                          price_ema)**2
        # Print the updated EMA price and volume
        # time_stamp = datetime.now(tzone).strftime("%Y-%m-%d %H:%M:%S")
        price_vol = np.sqrt(
            price_var
        )  # The price volatility is the square root of the price variance
        print(
            f"{time_stamp} {symbol} Price: {round(trade_price, ndigits=2)}, Size: {trade_size}, EMA price: {round(price_ema, ndigits=2)}, Volatility: {round(price_vol, ndigits=4)}"
        )
        ...
        if len(price_frame) > max_frame_size:
            price_frame = price_frame.tail(max_frame_size).reset_index(
                drop=True)
        return price_frame
    ...
    try:
        ...
        if hasattr(stream_client, '_ws') and stream_client._ws:
            if hasattr(stream_client._ws,
                       'is_running') and stream_client._ws.is_running:
